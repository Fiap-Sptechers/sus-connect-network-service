...
class ShiftServiceTest {
    ...
    void updateShift_ShouldSaveShiftWithCapacity() {
        ...
        ShiftUpdateRequest request = new ShiftUpdateRequest(unitId, SpecialtyEnum.CLINICA_GERAL.name(), 10);

        when(repository.findByUnitIdAndSpecialtyName(any(), any())).thenReturn(Optional.empty());
        when(healthUnitRepository.existsById(any())).thenReturn(true);
    ...
    void defineSchedule_ShouldSaveShift() {
        ...
        ShiftScheduleRequest request = new ShiftScheduleRequest(unitId, SpecialtyEnum.CLINICA_GERAL.name(), List.of(crm));
